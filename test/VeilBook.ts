...
//To run the test: npx hardhat test
 ...
 console.log("StateView contract address:", stateViewContractAddress);
  ...
  describe("afterSwap() settlement", function () {
    ...
    before(async function () {
      ...
      const currentTick = await hook.lastTick(poolId);
      ORDER_TICK_CLAIM = currentTick + 90n; // next valid tick above current
      console.log("Placing orders at tick:", ORDER_TICK_CLAIM);

      // Place sell order at tick 60
      let tx = await hook.connect(seller).placeOrder(
        poolKey, ORDER_TICK_CLAIM, true, encryptedInput0.handles[0], encryptedInput0.inputProof
      );
      ...
      tx = await hook.connect(buyer).placeOrder(
        poolKey, ORDER_TICK_CLAIM, false, encryptedInput1.handles[0], encryptedInput1.inputProof
      );
  ...
  describe("claimFill()", function () {
    ...
    before(async function () {
      ...
      const currentTick = await hook.lastTick(poolId);
      CLAIM_ORDER_TICK = currentTick + 100n; // next valid tick above current
      console.log("Placing orders at tick:", CLAIM_ORDER_TICK);

      // Place sell order at tick 60
      let tx = await hook.connect(seller).placeOrder(
        poolKey, CLAIM_ORDER_TICK, true, encryptedInput0.handles[0], encryptedInput0.inputProof
      );
      let receipt = await tx.wait();
      let event = receipt?.logs.find((log: any) => {
        try { return hook.interface.parseLog(log)?.name === "OrderPlaced"; }
      ...
      // Place buy order at tick 60
      tx = await hook.connect(buyer).placeOrder(
        poolKey, CLAIM_ORDER_TICK, false, encryptedInput1.handles[0], encryptedInput1.inputProof
      );
      receipt = await tx.wait();
      event = receipt?.logs.find((log: any) => {
        try { return hook.interface.parseLog(log)?.name === "OrderPlaced"; }
    ...
    it("seller should receive token1 after claimFill", async function () {
      ...
      const slot0After_ = await stateView.getSlot0(poolId);
console.log("Tick after swap:", slot0After_.tick.toString());
console.log("CLAIM_ORDER_TICK:", CLAIM_ORDER_TICK.toString());
console.log("Crossed:", slot0After_.tick >= CLAIM_ORDER_TICK);
